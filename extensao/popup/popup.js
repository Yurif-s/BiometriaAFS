...
function isAllowedPage(url) {
    return url.includes("professor.seduc.ce.gov.br") || url.endsWith("/index.html") || url.includes("/ScriptBiometriaAFS/index.html");
}
...
btn.onclick = async () => {
    ...
    if (!tab?.url || !isAllowedPage(tab.url)) {
        status.textContent = "Abra o Professor Online ou o index.html de teste.";
        return;
    }
